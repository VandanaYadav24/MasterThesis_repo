        ...
        for epoch in range(1, self.n_epochs + 1):
            ...
            if verbose > 1:
                ...
                self.print_metrics(eval_data=eval_data, metrics=metrics,
                                   **kwargs)
                print("=" * 30)
                train_loss_plt.append(round(float(np.mean(train_total_loss)), 4))
        x = np.arange(1, self.n_epochs+1, 1).tolist()
        plt.plot(x, train_loss_plt)
        plt.xlabel("no. of epochs")
        plt.ylabel("train loss")
        plt.xticks(x)
        plt.savefig('train_loss_vs_epoch_{}.png'.format(self.task), dpi=500)
